...
export function assertValidApproxDetail(
  ...
  if (approx.length !== detail.length) {
    throw new Error(
      'Approximation and detail coefficients must have equal length.',
    );
  }
  ...
  if (approx.length === 0) {
    throw new Error(
      'Approximation and detail coefficients must not have zero length.',
    );
  }
 ...
 */
export function assertValidFilters(filters: Readonly<Filters>): boolean {
  /* Check if high-pass and low-pass filters have equal length. */
  if (filters.high.length !== filters.low.length) {
    throw new Error(
      'High-pass and low-pass filters have to have equal length.',
    );
  }
  ...
  if (filters.low.length < 2) {
    throw new Error(
      'Wavelet filter length has to be larger than or equal to two.',
    );
  }
...
export function createArray(
  ...
  //   typeof populate === 'function' ? populate(index) : populate,
  // );
...
export function padWidths(
  ...
  if (dataLength <= 0) {
    throw new Error(
      'Cannot determine padding widths for data of length less than or equal to zero.',
    );
  }
  ...
  if (filterLength < 2) {
    throw new Error(
      'Cannot determine padding widths for filter of length less than two.',
    );
  }
...
export function waveletFromScalingNumbers(
  ...
  const waveletNumbers: ReadonlyArray<number> = scalingNumbers
    .slice() // Copy array
    .reverse()
